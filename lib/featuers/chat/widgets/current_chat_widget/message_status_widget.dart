...
class MessageStatusWidget extends StatelessWidget {
  ...
  Widget _buildMessageStatusIcon(BuildContext context) {
    ...
    Color iconColor =
        isDarkMode ? Colors.white70 : AppColors.textColor.textGreyColor;

    // ✅ DEBUG: Log message status for troubleshooting
    final messageStatus = chat.messageSeenStatus?.toLowerCase();
    debugPrint(
      '📱 UI DEBUG: Message ${chat.messageId} status: "$messageStatus"',
    );
    ...
    switch (messageStatus) {
      case 'seen':
        ...
        debugPrint(
          '📱 UI DEBUG: Message ${chat.messageId} showing BLUE ticks (seen)',
        );
        break;
      case 'sent':
      case 'delivered':
        ...
        iconColor =
            isDarkMode
                ? Colors.white70
                : AppColors.textColor.textGreyColor; // Grey when sent
        debugPrint(
          '📱 UI DEBUG: Message ${chat.messageId} showing GREY ticks (sent/delivered)',
        );
        break;
      case 'pending':
      case null:
      default:
        ...
        iconColor =
            isDarkMode ? Colors.white70 : AppColors.textColor.textGreyColor;
        debugPrint(
          '📱 UI DEBUG: Message ${chat.messageId} showing CLOCK (pending/unknown)',
        );
        break;
    }
  ...
  String _formatTime() {
    ...
    try {
      ...
      final timeString =
          '${hour12.toString()}:${dateTime.minute.toString().padLeft(2, '0')} $amPm';
