...
class CacheTestHelper {
  ...
  static Future<bool> testMultiplePageCaching() async {
    try {
      ...
      if (cachedPages.length != numPages) {
        developer.log(
          '❌ Multi-page test failed: Expected $numPages, got ${cachedPages.length}',
        );
        return false;
      }
      ...
      for (int page = 1; page <= numPages; page++) {
        ...
        if (messages == null || messages.length != (10 + page)) {
          developer.log(
            '❌ Multi-page test failed: Page $page retrieval failed',
          );
          return false;
        }
      }
    ...
    try {
      ...
      for (int i = 0; i < originalMessages.length; i++) {
        if (originalMessages[i].messageId != persistedMessages[i].messageId) {
          developer.log(
            '❌ Session persistence test failed: Message ID mismatch',
          );
          return false;
        }
      }
  ...
  static Future<void> demoCache() async {
    ...
    for (final page in cachedPages) {
      ...
      developer.log(
        '📖 Retrieved page $page: ${messages?.length ?? 0} messages',
      );
    }
