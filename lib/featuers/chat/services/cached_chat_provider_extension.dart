...
extension CachedChatProviderExtension on ChatProvider {
  ...
  Future<ChatsModel?> loadMessagesWithCache({
    ...
    try {
      developer.log(
        '🔍 Loading messages for chat $chatId, page $page (forceRefresh: $forceRefresh)',
      );
      ...
      if (!forceRefresh && _cacheService.hasPageInCache(chatId, page)) {
        ...
        if (cachedMessages != null && cachedMessages.isNotEmpty) {
          developer.log(
            '✅ Returning cached data for chat $chatId, page $page (${cachedMessages.length} messages)',
          );
      ...
      developer.log(
        '📡 Fetching fresh data from server for chat $chatId, page $page',
      );
  ...
  Future<Map<int, ChatsModel>> loadMultiplePagesWithCache({
    ...
    developer.log(
      '📊 Loaded ${result.length} pages for chat $chatId (${cachedPages.length} from cache, ${uncachedPages.length} from server)',
    );
  ...
  Future<void> loadMoreMessagesWithCache() async {
    try {
      ...
      if (nextPageData != null && nextPageData.records != null) {
        ...
        developer.log(
          '📄 Loaded page $nextPage with ${nextPageData.records!.length} messages',
        );
      }
    } catch (e) {
