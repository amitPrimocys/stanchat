...
Future<void> handleForwardMessagesReliable(
  ...
  try {
    ...
    for (int chatIndex = 0; chatIndex < chatIds.length; chatIndex++) {
      ...
      for (
        int msgIndex = 0;
        msgIndex < (selectedMessageIds?.length ?? 0);
        msgIndex++
      ) {
        ...
        try {
          debugPrint(
            '  📩 Forwarding message $messageId to chat $chatId (${msgIndex + 1}/${selectedMessageIds.length})',
          );
    ...
    for (int userIndex = 0; userIndex < userIds.length; userIndex++) {
      ...
      for (
        ...
        try {
          debugPrint(
            '  📩 Forwarding message $messageId to user $userId (${msgIndex + 1}/${selectedMessageIds.length})',
          );
    ...
    if (successCount == totalAttempts) {
      ...
      Navigator.of(
        context,
      ).popUntil((route) => route.isFirst); // Go to chat list
      ...
      showDialog(
        ...
        builder:
            (context) => AlertDialog(
              ...
              content: const Text(
                'Failed to forward any messages. Please try again.',
              ),
              actions: [
                TextButton(
...
///    import 'package:stanchat/featuers/contacts/screen/simple_forward_fix.dart';
///
/// 3. You can remove:
